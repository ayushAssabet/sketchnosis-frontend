...
const PatientActionForm: React.FC<{
  ...
  const { data, isLoading } = useGetPatientDetail(searchParams?.get("update"));
  ...
  const defaultCategories = useMemo(() => {
    return (
      data?.data?.areaOfConcerns?.map((category: Record<string, any>) => ({
        label: category?.name,
        value: category?.id,
      })) ?? []
    );
  ...
  return (
    <>
      ...
      >
        ...
        <CampaignSelector
          selectedCampaign={
            (data?.data?.campaign || formData?.campaigns[0]?.id) ?? ""
          }
          selectedStartDate={formData?.campaigns[0]?.startDate ?? ""}
        ...
        <AppInputField
          ...
          type="date"
          value={formData?.campaigns[0]?.startDate ?? ""}
          onChange={handleChange}
